/* -*-c++-*- */
/* $Header: h3cinit.c, 1, 9/11/99 10:13:20 PM PDT, StarTeam VTS Administrator$ */
/*
...
**
** $Revision: 1$
** $Date: 9/11/99 10:13:20 PM PDT$
**
** $History: h3cinit.c $
** 
** *****************  Version 3  *****************
** User: Andrew       Date: 8/31/99    Time: 11:03a
** Updated in $/devel/sst2/Win95/dx/hostvdd
** Fixed only have 85 Hz refresh rate bug
** 
** *****************  Version 2  *****************
** User: Peterm       Date: 6/25/99    Time: 2:24p
** Updated in $/devel/sst2/Win95/dx/hostvdd
** fixed modeset bug
** 
** *****************  Version 1  *****************
                    ...
                    FxU32 refresh)
{
    ...
    while (mode_table[i][0] != 0)
    {
        if ((mode_table[i][0] == xRes) &&
            (mode_table[i][1] == yRes) &&
            (mode_table[i][2] > tRefresh) &&
            (mode_table[i][2] <= 90))
        {
            tRefresh = mode_table[i][2];
            tMode = i;
        }
    ...
    // If any mode was found with the correct width and height return it
    if (tRefresh != 0)
        return &mode_table[tMode][3];

    // If no mode was found, return null
    return NULL;
#endif
}
