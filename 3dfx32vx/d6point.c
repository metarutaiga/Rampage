...
**
** $Log: 
**  18   3dfx      1.17        10/11/00 Brent Burton    DX8 code integration.  
**       Changes to headers, new code.
**  17   3dfx      1.16        9/14/00  Don Fowler      Specular was being added
**       to the base color of the point and then being alpha blended with the 
**       point later on. Removed the initial add so that the alpha blending was 
**       the only way specular was affecting the resultant color.
**  16   3dfx      1.15        7/24/00  Brian Danielson Changes to implement 
**       renderstate and shadow register management.
**  15   3dfx      1.14        7/10/00  Michel Conrad   Remove pixel offset 
**       cruft.
**  14   3dfx      1.13        6/15/00  Michel Conrad   Delete really old check 
**       in comments and drawglobal check.
**  13   3dfx      1.12        5/22/00  Evan Leland     removed dx7-specific 
**       ifdefs and code targeted to the pre-dx7 driver
**  12   3dfx      1.11        4/24/00  Brian Danielson Added Pint and Line fill
**       mode functionality. Also fixed some oversights I found in line and 
**       point code.
**  11   3dfx      1.10        2/9/00   Evan Leland     Minor change to some DX7
...
**  10   3dfx      1.9         1/27/00  Evan Leland     DX7 changes
**  9    3dfx      1.8         1/25/00  Evan Leland     updates for dx7
**  8    3dfx      1.7         1/25/00  Evan Leland     part of 
**       txtrCreateSurface reorg, texture struct integration, DX7 bring-up
**  7    3dfx      1.6         12/17/99 Brian Danielson Added FVF, Projected 
**       Textures, line&point VTA loop fix, PRS 11233 & 11290 fix, disbaled DX7 
**       Clear and remapped to ddiClear2, cleanups.
**  6    3dfx      1.5         10/12/99 Evan Leland     added NPT texture 
...
//---------------------------------------------------------------
//
//  dp2Point : Point drawing primitive
//             (used also by D3DFILL_WIREFRAME fill mode tri, fan, strip primitives)
//
void __stdcall dp2Point( RC *pRc, LPDWORD pF, LPDWORD pA, DWORD vertexType )
{
  SETUP_PPDEV(pRc)
  D3DCOLOR    aColor, saColor, aBaseColor;
  ...
  else
  {
    if ( pRc->shadeMode == D3DSHADE_FLAT )
        aBaseColor = pF[FVFO_COLOR];    // Use flat vlaue - strips and fans
    else
  ...
  if( pRc->state & STATE_REQUIRES_WBUFFER )
  {
    ...
    {
      z1 = (float)(255 - RGBA_GETALPHA(pA[FVFO_SPECULAR])) / 255.0f;	// this wasn't divided before - caused overflow - EJH
    }
  }
  else
  {
    ...
    if (pRc->state & STATE_REQUIRES_VERTEXFOG)
    {
      w1 = (float)(255 - RGBA_GETALPHA(pA[FVFO_SPECULAR])) / 255.0f;	// this wasn't divided before - caused overflow - EJH
    }
  ...
  // TODO: is +/- 0.5 OK for all points - or should we scale to VP height sometimes?
  vOffset = 0.5f;        

  for (vtx=0; vtx<2; vtx++)
  {
      //Send vertex 1st time
      if(su_mode & SST_SU_W)  {
      ...
      SETFPD( cmdFifo, ghw0->vpY, FLTP(pA)[FVFO_SY] + vOffset );
      if(su_mode & SST_SU_Z)
      ...
      #ifndef CMDFIFO    
      if (vtx==0) // send load index before vtx reloaded
      {
        P6FENCE;
