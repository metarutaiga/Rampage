...
**
** $Log: 
**  5    Rampage   1.4         12/7/00  Dale  Kenaston  Sage triangle fill mode 
**       rendering functions. Implemented the fill mode strip rendering 
**       function. Added locals to the solid fill function to support the macro 
**       changes.
**  4    Rampage   1.3         11/22/00 Dale  Kenaston  Sage packet 6b triangle 
**       list and strip/fan bug fix. Bug fix for VB 16bit indicies with an odd 
**       vertex count.
**  3    Rampage   1.2         11/22/00 Dale  Kenaston  Sage Non-Indexed VB 
**       Triangle Strip Bugfix. Added a call to the new VTX_INIT macro.
**  2    Rampage   1.1         11/22/00 Dale  Kenaston  Sage triangle fan 
**       primitive. Added vstart as a parameter to the rendering functions.
**  1    Rampage   1.0         11/20/00 Dale  Kenaston  
** $
*/
...
void __stdcall  geStripAllFill (RC *pRc, DWORD count, DWORD vertexType,
                                VINDEX idx,       DWORD istride,
                                LPDWORD vertices, DWORD vstride,
                                DWORD vstart)
{
    SETUP_PPDEV(pRc)
    int     iPrev2, iPrev, iNext;
    LPDWORD pPrev2, pPrev, pNext;
    LPDWORD pFirst;
    DWORD   i=0, j=0;
    DWORD   numVerts;
    ...
    for(; i<numVerts; i++)
    {
        ...
        case D3DFILL_WIREFRAME:
            //Draw the lines
            geLine( pRc, pFirst, pPrev2, pPrev, vertexType );
            geLine( pRc, pFirst, pPrev, pNext, vertexType );
            geLine( pRc, pFirst, pNext, pPrev2, vertexType );
            break;

        case D3DFILL_POINT:
            //Draw the points
            gePoint( pRc, pFirst, pPrev2, vertexType );
            gePoint( pRc, pFirst, pPrev, vertexType );
            gePoint( pRc, pFirst, pNext, vertexType );
            break;
...
void __stdcall geStripAll (RC *pRc, DWORD count, DWORD vertexType,
                           VINDEX idx,       DWORD istride,
                           LPDWORD vertices, DWORD vstride,
                           DWORD vstart)
{
    ...
    for( j=0; j<numVerts; j+=VERTEX_PER_PACKET )
    {
        ...
        SETPH(cmdFifo,
              CMDFIFO_BUILD_PK6_b(0,
                                  kPkt6bWindNormal,
                                  n,
                                  (j==0 ? kPkt6bCmdTStrip : kPkt6bCmdTStripC),
        ...
        for(i=0; i<n; i++)
        {
            ...
            if((iNext==iPrev)||(iNext==iPrev2)||(iPrev==iPrev2))
                _asm int 1;
